...
fn main() {
    ...
    // This can be replaced by an std::rc::Rc when Rust 1.16.2 is released in a few weeks.
    // See https://github.com/rust-lang/rust/pull/107317
    let uring = Arc::new(uring);

    // Create a new current_thread runtime that submits all outstanding submission queue
    // entries as soon as the executor goes idle. The use of SendWrapper here is safe as
    // long as the current_thread runtime is used.
